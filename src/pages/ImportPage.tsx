...
function buildBookmarklet(importUrlBase: string) {
  ...
  const js = `(function(){function t(e){return e?(e.innerText||e.textContent||'').trim():''}function name(){return document.title.replace(/-.*$/,'').trim()}function guess(){let a='',o='';Array.from(document.querySelectorAll('div,span,p,li')).forEach(function(el){const s=t(el);if(!a&&/(号.*室|层.*室|号.*楼)/.test(s))a=s;if(!o&&/(营业中|休息中|营业时间|\\d{1,2}:\\d{2}\\s*[-~到]\\s*\\d{1,2}:\\d{2})/.test(s))o=s});let p='';Array.from(document.querySelectorAll('div,span,p')).forEach(function(el){const s=t(el);if(!p&&/(人均|消费)[:：]?\\s*¥?\\s*\\d+/.test(s))p=s});let img='';const im=document.querySelector('img');if(im&&im.src)img=im.src;return{address:a,opening:o,priceLine:p,imageUrl:img}}const g=guess();const payload={sourceUrl:location.href,name:name(),address:g.address,openingHours:g.opening,priceSingle:(g.priceLine.match(/\\d+/)||[])[0]?Number((g.priceLine.match(/\\d+/)||[])[0]):null,priceNote:g.priceLine||'',imageUrl:g.imageUrl||''};const json=JSON.stringify(payload);const b64=btoa(String.fromCharCode.apply(null,new TextEncoder().encode(json)));location.href='${importUrlBase}#'+encodeURIComponent(b64);})();`
...
export function ImportPage() {
  ...
  async function save() {
    ...
    try {
      ...
      setResult(`已写入 gyms：${row.id}，即将返回首页…`)
