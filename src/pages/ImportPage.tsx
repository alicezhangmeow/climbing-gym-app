...
function buildBookmarklet(importUrlBase: string) {
  ...
  const js = `(function(){function txt(el){return el?(el.innerText||el.textContent||'').trim():''}function firstLine(s){return (s||'').split(/\\n/)[0].trim()}function pickNameFromTitle(){var t=firstLine(document.title||'');t=t.replace(/-.*$/,'').trim();var m=t.match(/【([^】]+)】/);if(m&&m[1])return m[1].trim();t=t.split('_')[0].trim();t=t.split('电话')[0].trim();t=t.split('地址')[0].trim();t=t.split('价格')[0].trim();t=t.split('营业时间')[0].trim();return t.trim()}function pick(list,maxLen){for(let i=0;i<list.length;i++){const s=list[i];if(!s)continue;const v=s.trim();if(!v)continue;if(maxLen&&v.length>maxLen)continue;if(/(点评|收藏|热门榜|榜单|更多|展开|收起)/.test(v))continue;return v}return ''}function numFrom(s){const m=(s||'').match(/¥\\s*(\\d{1,4})/);return m&&m[1]?Number(m[1]):null}function districtFrom(addr){const m=(addr||'').match(/(.{2,6}区)/);return m&&m[1]?m[1]:''}function ctx(el){if(!el)return '';var p=el.parentElement;var s=txt(el);if(p)s+=' '+txt(p);if(p&&p.parentElement)s+=' '+txt(p.parentElement);var prev=el.previousElementSibling;var next=el.nextElementSibling;if(prev)s+=' '+txt(prev);if(next)s+=' '+txt(next);return s}function isTicketContext(s){return /(团购|门票|成人|不限时|单人|攀岩|入场|日票|票)/.test(s)&&!/(租|装备|鞋|镁|水|饮料|粉|课程|私教|课包)/.test(s)}function guess(){const nodes=Array.from(document.querySelectorAll('div,span,p,li'));const addrCandidates=[];const openCandidates=[];const avgCandidates=[];const priceEls=[];for(const el of nodes){const s=txt(el);if(!s)continue;if(s.length>160)continue;if(/(号.*(室|楼)|\\d+号)/.test(s)&&/(路|街|大道|弄|号)/.test(s))addrCandidates.push(s);if(/(营业中|休息中)/.test(s)||/\\d{1,2}:\\d{2}\\s*[-~到]\\s*\\d{1,2}:\\d{2}/.test(s))openCandidates.push(s);if(/(人均|消费)[:：]?\\s*¥?\\s*\\d+/.test(s))avgCandidates.push(s);if(/¥\\s*\\d{1,4}/.test(s))priceEls.push(el)}let address=pick(addrCandidates,48);let opening=pick(openCandidates,48);if(opening){const m=opening.match(/(\\d{1,2}:\\d{2})\\s*[-~到]\\s*(\\d{1,2}:\\d{2})/);if(m)opening='每日 '+m[1]+'–'+m[2]}let priceLine=pick(avgCandidates,32);let priceSingle=numFrom(priceLine);if(priceSingle==null){let best=null;for(const el of priceEls){const c=ctx(el);if(!isTicketContext(c))continue;const n=numFrom(c)||numFrom(txt(el));if(n==null)continue;if(n<20)continue;if(best==null||n<best){best=n}}if(best!=null){priceSingle=best}}let area=districtFrom(address);let img='';const im=document.querySelector('img');if(im&&im.src)img=im.src;return{area:area,address:address,opening:opening,priceSingle:priceSingle,imageUrl:img}}const g=guess();const payload={sourceUrl:location.href,name:pickNameFromTitle(),area:g.area,address:g.address,openingHours:g.opening,priceSingle:g.priceSingle,imageUrl:g.imageUrl||''};const json=JSON.stringify(payload);const b64=btoa(String.fromCharCode.apply(null,new TextEncoder().encode(json)));location.href='${importUrlBase}#'+encodeURIComponent(b64);})();`
